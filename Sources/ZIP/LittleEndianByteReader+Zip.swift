...
extension LittleEndianByteReader {
    ...
    func zipString(_ length: Int, _ useUtf8: Bool) -> String? {
        ...
        if String.cp437Available && !stringData.needsUtf8() {
            #if (os(Linux) || os(Windows)) && (compiler(<6.2))
                return stringData.decodeCp437()
            #else
...
fileprivate extension String {
    ...
    // control characters.
    #if (os(Linux) || os(Windows)) && (compiler(<6.2))
        /// CP437 is available on all platforms where decoding is performed manually.
        static let cp437Available = true
        static let cp437UnicodeScalars = [0x0000, 0x263A, 0x263B, 0x2665, 0x2666, 0x2663, 0x2660, 0x2022, 0x25D8, 0x25CB,
                                          ...
                                          0x00B7, 0x221A, 0x207F, 0x00B2, 0x25A0, 0x00A0].map { UnicodeScalar($0)! }
    #else
...
fileprivate extension Data {
    ...
    #if (os(Linux) || os(Windows)) && (compiler(<6.2))
        func decodeCp437() -> String {
            var out = ""
            out.unicodeScalars.append(contentsOf: self.toByteArray().map { String.cp437UnicodeScalars[$0.toInt()] })
            return out
        }
    #endif
